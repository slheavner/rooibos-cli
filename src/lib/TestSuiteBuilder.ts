...
export class TestSuiteBuilder {
  constructor(
    maxLinesWithoutSuiteDirective: number,
    isLegacyTestsSupported: boolean
  ) {
    this._maxLinesWithoutSuiteDirective = maxLinesWithoutSuiteDirective;
    this.functionNameRegex = new RegExp(
      "^\\s*(function|sub)\\s*([0-9a-z_]*)s*\\(",
      "i"
    );
    this.functionSignatureRegex = new RegExp(
      "^\\s*(function|sub)\\s*[0-9a-z_]*s*\\((.*)\\)",
      "i"
    );
    this.assertInvocationRegex = new RegExp(
      "^\\s*(m\\.fail|m\\.assert)(.*)\\(",
      "i"
    );
  ...
  public processFile(file: File): TestSuite {
    ...
    if (!code || !code.trim()) {
      ...
      this.errors.push(
        "No code for file" + (file ? file.fullPath : `unknown file`)
      );
      return testSuite;
    }
    ...
    for (let lineNumber = 1; lineNumber <= lines.length; lineNumber++) {
      ...
      if (this.isTag(line, Tag.TEST_SUITE)) {
        if (isTestSuite) {
          ...
          this.warnings.push(
            "Multiple suites per file are !supported - use '@It tag" +
              currentLocation
          );
        }
        ...
        this.isTag(line, Tag.SOLO) &&
        !this.isTag(line, Tag.TEST_SOLO_PARAMS)
      ) {
        if (isNextTokenSolo) {
          debug(`Tag.TEST MARKED FOR Tag.IGNORE AND Tag.SOLO`);
          this.warnings.push(
            `Tag.TEST MARKED FOR Tag.IGNORE AND Tag.SOLO ${currentLocation}`
          );
        } else {
        ...
        this.isTag(line, Tag.IGNORE) &&
        !this.isTag(line, Tag.TEST_IGNORE_PARAMS)
      ) {
        ...
        if (isTestSuite) {
          ...
          debug(
            `If you wish to run this suite of tests on a node, then make sure the ` +
              Tag.NODE_TEST +
      ...
      if (
        ...
        if (functionName) {
          let numberOfExpectedParams =
            this.getNumberOfExpectedParamsFromLine(line);
          if (isNextTokenTest) {
            let testName = nextName || functionName;
            nodeTestFileName = nodeTestFileName || testSuite.nodeTestFileName;

            if (
              this.testCaseParams.length > 0 ||
              ...
              for (let index = 0; index < paramsToUse.length; index++) {
                ...
                if (testCase) {
                  ...
                  debug(
                    `Skipping unparseable params for testcase ` +
                      params +
                      ` @` +
                      currentLocation
                  );
            ...
            if (this.currentGroup) {
              ...
              this.warnings.push(
                `There is no currentGroup! - ignoring test ${currentLocation}`
              );
              continue;
            }
            ...
            debug(
              ` could !get function pointer for ` + functionName + ` ignoring`
            );
            this.errors.push(
              ` could !get function pointer for ` +
                functionName +
          ...
          this.warnings.push(
            `could not find function directly after '@Setup - ignoring: ${currentLocation}`
          );
          isNextTokenSetup = false;
        } else if (isNextTokenTearDown) {
          debug(`could not find function directly after '@TearDown - ignoring`);
          this.warnings.push(
            `could not find function directly after '@TearDown - ignoring: ${currentLocation}`
          );
          isNextTokenTearDown = false;
        } else if (isNextTokenBeforeEach) {
          debug(
          ...
          this.warnings.push(
            `could not find function directly after '@BeforeEach - ignoring: ${currentLocation}`
          );
          isNextTokenBeforeEach = false;
        } else if (isNextTokenAfterEach) {
          debug(
          ...
          this.warnings.push(
            `could not find function directly after '@AfterEach - ignoring: ${currentLocation}`
          );
          isNextTokenAfterEach = false;
        } else if (isNextTokenSetup) {
          debug(`could not find setup function - ignoring '@Setup`);
          this.warnings.push(
            `could not find setup function - ignoring '@Setup: ${currentLocation}`
          );
          isNextTokenSetup = false;
    ...
    if (!isTestSuite) {
      ...
      if (this._isLegacyTestsSupported) {
        debug(
          "legacy tests are supported - checking if this file contains legacy tests"
        );
  ...
  public addParamsForLine(
    file: File,
    line: string,
    tag: Tag,
    lineNumber: number,
    targetParamLinesArray: number[],
    targetParamsArray: object[],
    currentLocation: string
  ) {
    let rawParams = this.getTagText(line, tag);
    try {
      let jsonText = rawParams.replace(
        this.paramsInvalidToNullRegex,
        "$1$2null"
      );
      ...
      if (jsonParams) {
        ...
        feedbackError(
          file,
          `illegal params found at ${currentLocation}. Not adding test - params were : ${line}`
        );
      }
    } catch (e) {
      feedbackError(
  ...
  public processLegacyFile(file: File, testSuite: TestSuite): boolean {
    ...
    let testSuiteFunctionNameRegex = new RegExp(
      "^\\s*(function|sub)\\s*testSuite_([0-9a-z\\_]*)\\s*\\(",
      "i"
    );
    let testCaseFunctionNameRegex = new RegExp(
      "^\\s*(function|sub)\\s*testCase_([0-9a-z\\_]*)\\s*\\(",
      "i"
    );
    let assertInvocationRegex = new RegExp(
      "^.*(m.fail|m.Fail|m.assert|m.Assert)(.*)",
      "i"
    );
    let functionEndRegex = new RegExp("^\\s*(end sub|end function)", "i");

    let testSuiteNameRegex = new RegExp(
      '^\\s*this\\.name\\s*=\\s*"([0-9a-z_]*)"',
      "i"
    );
    let setupRegex = new RegExp("^\\s*this\\.setup\\s*=\\s*([a-z_0-9]*)", "i");
    let teardownRegex = new RegExp(
      "^\\s*this\\.tearDown\\s*=\\s*([a-z_0-9]*)",
      "i"
    );
    let addTestregex = new RegExp(
      '^\\s*this\\.addTest\\s*\\(\\s*"([0-9a-z_]*)"\\s*,\\s*([0-9a-z_]*)s*',
      "i"
    );
    ...
    for (let lineNumber = 1; lineNumber <= lines.length; lineNumber++) {
      ...
      if (lineNumber > this._maxLinesWithoutSuiteDirective && !isTestSuite) {
        debug(
          "IGNORING FILE WITH NO LEGACY TESTS AVAILABLE : " + currentLocation
        );
        this.warnings.push(
          "Ignoring file with no legacy tests" + file.fullPath
        );
        break;
      }
      ...
      if (line.match(testSuiteFunctionNameRegex)) {
        ...
        if (testCaseName !== "" && testCaseFunctionName !== "") {
          ...
          debug(
            " found badly formed addTestCase function call in test suite init function. Ignoring"
          );
        }
